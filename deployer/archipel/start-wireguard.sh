...
if [ ! -z "$CONFIG_FILE" ]; then
    ...
    if [ -f "/config/archipel-config.zip" ]; then
        echo "start-wireguard.sh : unzip /config/archipel-config.zip"
        #if config file password was set unzip with password
        if [ ! -z "$CONFIG_FILE_PASSWORD" ]; then
            echo "start-wireguard.sh : unzip if first time  with -u option."
            unzip -u -P "$CONFIG_FILE_PASSWORD" -o /config/archipel-config.zip -d /config
            echo "start-wireguard.sh : refresh all configs files with unzip -f option."
        ...
        echo "start-wireguard.sh : no file /config/archipel-config.zip"
    fi
